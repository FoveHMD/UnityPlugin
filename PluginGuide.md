...
* *Source*: We are releasing the source for our plugin so that you can see what's going on, make changes if you don't like something, and suggest improvements if you feel so inclined. All our source for the plugin is contained beneath this folder.
  * *Behaviours*:
    ...
    * *FoveManager*: This is a singleton behaviour which is automatically created whenever you try to get data from the FOVE SDK. Global settings and information can be accessed from this class using the `FoveManager.Instance` object.
    * *GazableObject*: Coupled with a Unity collider this, this component registers the object it is attached to into the Fove system to perform automatic gaze object detection.
  * *Classes*: Several data class used by the plugin
...
If enabled a camera object is automatically registered for the camera associated to this `FoveInterface`.
...
Allow you to remove specific layer of object from the corresponding eye image. Note that those per-eye culling masks are applied on top of the camera default culling mask. Modifying the eye culling mask of an object layer that is not initially rendered by the camera won't have any effect.
...
* *Automatic Gaze Object Registration*: If checked, any game object having a collider is automatically registered as gazable object. This allow you to query game object that the user is currently gazing at using the `FoveManager.GetGazedObject`. Read [Identify which object the user is looking at](#identify-which-object-the-user-is-looking-at) for more details.
...
* *Gaze Cast Policy*: Specify how the different gaze cast functions should behave when the user is closing one or two eye. The default behavior is the dismiss collision detection when the user is closing both eye but `Never dismiss` can be particularly useful during debug in order to be able to trigger collision without having to wear the headset.
...
This is a singleton class automatically created when starting your game. The singleton instance can be accessed through the `Instance` static field of the class. 

It allows you to modify the Fove settings at runtime and to retrieve gaze information that is not dependent of any camera, such as:
* Calibration status
* EyeClosed status
* Gazed object
* Raw gaze vectors
* Raw pose information
* Headset Taring functions
* Software versions
* etc.
---
...
### Get gaze vector information

Gaze vector information can either be accessed from the `FoveInterface` using the `GetGazeRays` or `GetGazeConvergence` functions, or from the `FoveManger` using the `GetHMDGazeConvergence`, `GetLeftEyeVector` or `GetRightEyeVector` functions. The difference is that data returned by the `FoveInterface` are in the world space coordinate while data returned by the `FoveManager` are in the headset space coordinate.

---

### Know the Object gazed by the user

There are mainly two ways to detect which object the user is looking at: a higher level API and a lower level API.
We recommend developers to use the higher level API as much as possible rather than performing manual ray casting as it is more robust, allow us to also improve the eye tracking and will be continuously improved.

#### Automatic gazed object detection

From plugin version 3.2, we added the `GetGazedObject` function on the `FoveManager` class. This function returns the Unity game object that the user is currently looking at.

For this function to work properly, you need to add a `GazableObject` component as well as an Unity collider (currently we support only Sphere, Cube and Mesh colliders) on objects that we want to be gazable. All the object registration and update is automatically done the `GazableObject`. Nevertheless, as Unity do not provide callbacks for object state changes manual action is needed when you change the collider at runtime. In this case, to update the scene object registered shape, call `GazableObject.RebuildShape`.
...
- `Rigidbody`: Use this if the position of your object is coming from a rigitbody.

To disable the detection of specific objects you have several options:
- Using the `FoveInterface.GazeCastCullMask` you can disable detection with all the objects of a given object layer.
...
If you don't want to manually create all the gazable object of your scene, we also provide a few alternative way to create them automatically from existing colliders:
- Fove settings: `Automatic Gaze Object Registration` automatically creates `GazableObject` components for all the collider objects of your scene. This option is here for convenience, but we do not recommend to use it in big-scale game but rather to manually register only objects of interest.
...
Also in a similar way to the Gaze Object Detection feature, the GazeCastPolicy property of the in the Fove settings allow you to choose to ignore or not gaze cast collisions when the user is closing one or two eyes.

If you want to have even more control on what you are doing, you can also grab the gaze rays as UnityEngine `Ray` objects from the method `GetGazeRays()` on your `FoveInterface` instance.

---

### Move the camera around in the world

The FOVE tracks your position and orientation and your `FoveInterface` behaviour will automatically position itself in accordance. What this means for you, dear dev, is that your `GameObject` is going to be spending a lot of time near (but generally not quite at) its origin.
...
We fully support using full screen image effects in all supported versions of Unity. Under most use cases, everything should appear in each eye exactly how it appears in the Unity Editor.
...
By default the Fove plug-in optimizes the rendering and copies the HMD view as-is onto the desktop display. If you want to have some configuration settings or show a different view of your scene on your PC monitor, you can disable this optimization from the Fove settings. Open the Resources/FOVE Settings file in Unity editor and check the `Custom Desktop View` option. After enabling this option, enabled cameras will render to your desktop view while only the cameras (enabled or not) with a `FoveInterface` component will render to the HMD. So: 
- To render objects only on the HMD, disable the camera component rendering those objects. 
- To render objects only on the Desktop display, add the object layers to the `Per-Eye Culling Masks` of the fove interface or disable/remove the `FoveInterface` component of the camera.
- To display some objects on both and some only on one, split your objects into several layers and adjust the fove interface and camera Culling Masks accordingly.
...
If you want to get the immediate, most up-to-date, current information from the SDK, you simply set the `immediate` parameter to `true` when accessing `FoveInterface` or `FoveManager` functions such as:
